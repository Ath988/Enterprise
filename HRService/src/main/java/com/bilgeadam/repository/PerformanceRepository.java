...
public interface PerformanceRepository extends JpaRepository<Performance,Long> {
    ...
    @Query("SELECT NEW com.bilgeadam.dto.response.PerformanceResponse(p.id,p.grade,p.feedBack,p.trainingHours,p.trainingTopics,p.createAt) FROM Performance p WHERE p.id = ?1")
