...
public interface EmployeeRepository extends JpaRepository<Employee,Long> {
    ...
    @Query("SELECT NEW com.bilgeadam.view.VwEmployee(e.id,CONCAT(e.firstName,' ',e.lastName),e.avatarUrl) FROM Employee e JOIN Position p ON p.id = e.positionId JOIN Department d ON p.departmentId = d.id WHERE d.managerId = e.id AND d.id = ?1")
    Optional<VwEmployee> findVwManagerByDepartmentId(Long departmentId);

    @Query("SELECT NEW com.bilgeadam.view.VwEmployee(e.id,CONCAT(e.firstName,' ',e.lastName),e.avatarUrl) FROM Employee e JOIN Position p ON p.id = e.positionId JOIN Department d ON d.id = p.departmentId " +
    ...
    @Query("SELECT NEW com.bilgeadam.view.VwEmployee(e.id,CONCAT(e.firstName,' ',e.lastName),e.avatarUrl) FROM Employee e JOIN Position p ON p.id = e.positionId where e.positionId = ?1")
