...
public class GenerateData {
    ...
    public void init(){
        ...
        if(userRepository.count() == 0 ){
            User member = User.builder().authId(1L).companyId(1L).email("vehbi@test.com").firstName("Vehbi").lastName("Koç").gender(EGender.MALE).build();
            User user1 = User.builder().authId(2L).companyId(1L).email("hasan@test.com").firstName("Hasan").lastName("Kayar").gender(EGender.MALE).build();
            User user2 = User.builder().authId(3L).companyId(1L).email("ayse@test.com").firstName("Ayse").lastName("Kulin").gender(EGender.FEMALE).build();
            User user3 = User.builder().authId(4L).companyId(1L).email("mehmet@test.com").firstName("Mehmet").lastName("Oz").gender(EGender.MALE).build();
            User user4 = User.builder().authId(5L).companyId(1L).email("hulya@test.com").firstName("Hulya").lastName("Pamuk").gender(EGender.FEMALE).build();
            ...
            //Member
            UserRolePermission urp1 = UserRolePermission.builder().role(roleRepository.findByName("MEMBER").get())
                    .permissions(Set.of(Permission.ACCESS_ALL_MODULES)).user(member).build();
            //Employee
            UserRolePermission urp2  = UserRolePermission.builder().role(roleRepository.findByName("STAFF").get())
                    .permissions(employeePermissions).user(user1).build();
            UserRolePermission urp3  = UserRolePermission.builder().role(roleRepository.findByName("STAFF").get())
                    .permissions(employeePermissions).user(user2).build();
            UserRolePermission urp4  = UserRolePermission.builder().role(roleRepository.findByName("STAFF").get())
                    .permissions(employeePermissions).user(user3).build();
            UserRolePermission urp5  = UserRolePermission.builder().role(roleRepository.findByName("STAFF").get())
